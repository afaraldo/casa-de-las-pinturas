DevolucionVentasUI= (function(){
    ...
    function seleccioneCliente(){
        elementos.mensajePanel.find('h3').text('Seleccione un cliente para ver las boletas para la devolucion');
     ...
     * @param cliente
     */
    function noHayBoletas(cliente){
        elementos.mensajePanel.find('h3').text('No hay boletas para el cliente ' + cliente);
    ...
    function initFormEvents(){
        ...
        elementos.devolucionVentaForm.validate({ignore: []}); // validar formulario. ignore: [] es para que valide campos no visibles tambien
    ...
    // se asume que los inputs tengan las clase .cliente-select
    function eliminarItemsSeleccionados(items) {
        var seleccionados = $.map($('input.cliente-select'), function(v,i){ return $(v).val();});
